		...
		<div class="row">
			<div class="col-md-12">
				...
				<c:if test="${not empty save}">
					...
					<h3 class="alert alert-success">${save}</h3>

				</c:if>
				
				<c:if test="${not empty delete}">

					<h3 class="alert alert-danger">${delete}</h3>

				</c:if>
				<c:if test="${not empty update}">

					<h3 class="alert alert-warning">${update}</h3>
						...
						<c:forEach items="${produtos}" var="produto">
							<tr>
								...
								<td class="actions"><form:form
										action="${contextPath}/delete/${produto.id}" method="delete">

										<a class="btn btn-success btn-xs"
											href="${contextPath}/produto/${produto.id}">Detalhes</a>
										<a class="btn btn-warning btn-xs"
											href="${contextPath}/update/${produto.id}">Editar</a>
